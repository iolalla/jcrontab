...
public class CrontabT
{
    ...
    public void init(String strFileName, int iTimeTableGenerationFrec)
        ...
        cron = new CronT(this, iTimeTableGenerationFrec);
     ...
     * is called from method run() of Thread when a task has finished.
     ...
     */
    public Thread getTask(int iTaskID) {
        synchronized(tasks) {
     ...
     */
    public Thread[] getAllTasks() {
        Thread[] t;
        synchronized(tasks) {
            int i = 0;
            t = new Thread[tasks.size()];
            Iterator iter = tasks.values().iterator();
            while(iter.hasNext()) {
     ...
     */
    public Thread[] getTasksOfClass(String strClassName) {
        Thread[] t;
        TaskTableEntry tmp;
        synchronized(tasks) {
            int i = 0;
