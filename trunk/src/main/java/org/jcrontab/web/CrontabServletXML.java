...
/**
 * This Servlet writes in xml format all the CrontabEntryBean availables. This
 * represents the contoller and the actions in a MVC pattern Usually this
 * servlet is used tiwh a xsl file to generate the final HTML
 * 
...
public class CrontabServletXML extends HttpServlet {
	 ...
	 * 
	 * @param request
	 *            This is the servlet request. refer to the Servlet JavaDoc
	 * @param response
	 *            This is the servlet response refer to the Servlet JavaDoc
	 */
	...
	/**
	 * This method processes the POST information, and saves the info comming
	 * from the web
	 * 
	 * @param request
	 *            This is the servlet request. refer to the Servlet JavaDoc
		...
		if (request.getParameterValues("remove") == null) {
			...
			try {
				for (int i = 0; i < idToDelete.length; i++) {
					int parsedInd = Integer .parseInt(idToDelete[i]);
					CrontabEntryBean resulti = daoTmp.getById(parsedInd);
					result[i] = resulti;
				}
	...
	/**
	 * This method transforms the xml/xsl and prints the whole thing in order to
	 * get hte HTML page. Should be called the last.
	 * 
	 * @param request
	 *            This is the servlet request. refer to the Servlet JavaDoc
	 * @param response
	...
	public void show(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			out = new PrintWriter(response.getWriter(),true);
			...
			for (int i = 0; listOfBeans!=null && i < listOfBeans.length; i++) {
				...
				String xml = crontabEntryBean.toXML();
				sb.append(xml);
			}
			sb.append("</crontabentries>");
			...
			Source xmlsource = new StreamSource(new StringReader(
					(String) sb.toString()));
			/*
			 * The following lines allow to acces to the correctxsl file it's
			 * quite tricky and surelly should beother ways to do in the easy
			 * way
			 */
